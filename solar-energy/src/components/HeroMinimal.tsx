...
const HeroMinimal = forwardRef<HTMLElement, HeroMinimalProps>(function HeroMinimal(
  { imageUrls }: HeroMinimalProps,
  ref
) {
  ...
  return (
    <motion.section ref={ref as any} className="relative overflow-hidden" initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}>
      ...
      <div className="fixed right-0 bottom-0 z-50 text-right mr-[-2px] mb-[-2px]">
        <button onClick={()=>setBubble(v=>!v)} aria-label="Assistente" className="mascot-stand">
          <img src="https://i.postimg.cc/Y9KQgcw6/Design-sem-nome-2025-08-18-T120443-767.png" alt="Mascote" className="h-[140px] sm:h-[200px] md:h-[270px] w-auto bob"/>
        </button>
        <AnimatePresence>
          {bubble && (
